...
private:
  ...
  void JointState(const sensor_msgs::JointState& jointstate)
  {
    for(size_t i = 0; i < jointstate.name.size(); i++)
    {
      ...
      else if(jointstate.name[i] == "shoulder_lift_joint")
      {
        shoulder_lift_joint = jointstate.position[i];
      }
      else if(jointstate.name[i] == "elbow_joint")
      {
        elbow_joint = jointstate.position[i];
      }
      else if(jointstate.name[i] == "wrist_1_joint")
      {
        wrist_1_joint = jointstate.position[i];
      }
      else if(jointstate.name[i] == "wrist_2_joint")
      {
        wrist_2_joint = jointstate.position[i];
      }
      else if(jointstate.name[i] == "wrist_3_joint")
      {
      ...
      {
        ROS_ERROR_STREAM(
          "Could not understand joint name '" << jointstate.name[i] << "'");
      }
    }
  }
  ...
  void SetPoint(const jps_traveler::MotionWithTime& goal_msg)
  {
    ...
    tf::Quaternion quatgoal(
      goaltransform.orientation.x, goaltransform.orientation.y,
      goaltransform.orientation.z, goaltransform.orientation.w);

    Eigen::Vector3d posgoal(
      goaltransform.position.x, goaltransform.position.y,
...
public:
  Trajectory(ros::NodeHandle& nh) : nh(nh)
  {
    sub_setpoint = nh.subscribe("setpoint", 1, &Trajectory::SetPoint, this);
    sub_jointstate =
      nh.subscribe("joint_states", 1, &Trajectory::JointState, this);
    // pub_trajectory = nh.advertise <
    // trajectory_msgs::JointTrajectory>("/arm_controller/follow_joint_trajectory",
    // 1); action_client = new
    // actionlib::SimpleActionClient<control_msgs::FollowJointTrajectoryAction>("/arm_controller/follow_joint_trajectory",
    // // For simulation
    // true);
    action_client =
      new actionlib::SimpleActionClient<control_msgs::FollowJointTrajectoryAction>(
        "/joint_trajectory",  // For simulation
        true);
