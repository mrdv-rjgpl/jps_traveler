...
void forwardkin(StampedTransform t, Eigen::Matrix(double,6,1) jointval)
	...
	*q=jointval[0];
	...
	t[0][1] = (c6*(s1*s5 + ((c1*c234-s1*s234)*c5)/2.0 + ((c1*c234+s1*s234)*c5)/2.0) - 
          (s6*((s1*c234+c1*s234) - (s1*c234-c1*s234)))/2.0); 
    t[0][2] = (-(c6*((s1*c234+c1*s234) - (s1*c234-c1*s234)))/2.0 - 
    ...
    t[1][1] = (c6*(((s1*c234+c1*s234)*c5)/2.0 - c1*s5 + ((s1*c234-c1*s234)*c5)/2.0) + 
          s6*((c1*c234-s1*s234)/2.0 - (c1*c234+s1*s234)/2.0)); 
    t[1][2] = (c6*((c1*c234-s1*s234)/2.0 - (c1*c234+s1*s234)/2.0) -
